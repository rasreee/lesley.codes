const group = {
  ...
  disabled: (selector: string) => `${selector}:disabled &, ${selector}[data-disabled] &`,
  invalid: (selector: string) => `${selector}:invalid &, ${selector}[data-invalid] &`,
  checked: (selector: string) => `${selector}:checked &, ${selector}[data-checked] &`,
...
const toGroup = (fn: AnyFunction) => merge(fn, '[role=group]', '[data-group]', '.group');
...
export const pseudo = (key: typeof pseudoPropNames[number]) => {
  return pseudoSelectors[key];
};
